    ...
    , m_openInNewTab(false)
{
    ...
    setNetworkAccessManager(VisingoApp::networkAccessManager());
#endif
...
BrowserMainWindow *WebPage::mainWindow()
{
    ...
    return VisingoApp::instance()->mainWindow();
}
...
QWebEnginePage *WebPage::createWindow(QWebEnginePage::WebWindowType type)
{
    if (m_openInNewTab || type == QWebEnginePage::WebBrowserTab) {
        ...
        BrowserMainWindow *mainWindow = VisingoApp::instance()->mainWindow();
...
QIcon WebView::icon() const
{
    if (!m_icon.isNull())
        return m_icon;
    return VisingoApp::instance()->defaultIcon();
}

void WebView::onIconUrlChanged(const QUrl &url)
{
    QNetworkRequest iconRequest(url);
    m_iconReply = VisingoApp::networkAccessManager()->get(iconRequest);

...
WebView *TabWidget::newTab(bool makeCurrent)
{
    ...
    if (!m_lineEditCompleter && count() > 0) {
        ...
        completionModel->setSourceModel(VisingoApp::historyManager()->historyFilterModel());
...
void TabWidget::webViewTitleChanged(const QString &title)
{
    ...
    VisingoApp::historyManager()->updateHistoryItem(webView->url(), title);
}
...
void TabWidget::aboutToShowRecentTabsMenu()
{
    ...
    for (int i = 0; i < m_recentlyClosedTabs.count(); ++i) {
        ...
        QIcon icon = VisingoApp::instance()->icon(m_recentlyClosedTabs.at(i));
        action->setIcon(icon);
...
{
    VisingoApp::downloadManager()->download(download);
    download->accept();
}
